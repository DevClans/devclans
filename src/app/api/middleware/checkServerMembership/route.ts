...
export default async function checkServerMembership(req: NextRequest, res: NextResponse) {
    ...
    if (isMember) {
      ...
      return new NextResponse('User is a member of the required Discord server', { status: 200 });
    } else {
      // User is not a member of the required Discord server
      return new NextResponse('User is not a member of the required Discord server', { status: 403 });
    }
  }
