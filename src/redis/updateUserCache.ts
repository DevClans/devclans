...
export default async function updateAllCache(
  ...
  updateThemeCache: boolean = false
) {
  try {
    ...
    const ZodDataSchema = isProject ? zodProjectDataSchema : zodUserDataSchema;
    ...
    if (!isProject && updateThemeCache) {
      themeData = zodUserTheme.partial().parse(formData);
    }
    console.log(" data:", Boolean(dataCache));
    console.log(" list data:", Boolean(searchInfoCache));
    ...
    if (themeData) {
      ...
      pipeline.set(
        UserRedisKeys.links + ":" + themeData.username,
